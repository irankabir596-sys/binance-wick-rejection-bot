...
def build_message(symbol: str, timeframe: str, signal: str, level: Level, current_price: float) -> str:
    ...
    tv_interval = {"1D": "D", "4H": "240", "1H": "60", "15M": "15"}.get(timeframe, "60")
    tv_url = f"https://www.tradingview.com/symbols/{coin}USDT/?exchange={TRADINGVIEW_EXCHANGE}&interval={tv_interval}"
    return (
        f"{icon} <b>{coin}</b> | <b>{timeframe}</b>\n"
        f"{action}: <b>{format_price(level.price)}</b> {stars}\n"
        f"💰 Price: {format_price(current_price)}\n"
        f"📊 <a href=\"{tv_url}\">Trading view</a>"
    )


def state_key(symbol: str, timeframe: str, candle_open: int, signal: str, level: float) -> str:
    # Rounded level avoids duplicate keys caused by floating point representation.
    return f"{symbol}|{timeframe}|{candle_open}|{signal}|{round(level, 8)}"
...
def fetch_one(symbol: str, timeframe: str, interval: str, now_ms: int):
    try:
        ...
        key = state_key(symbol, timeframe, current.open_time, signal_name, level.price)
